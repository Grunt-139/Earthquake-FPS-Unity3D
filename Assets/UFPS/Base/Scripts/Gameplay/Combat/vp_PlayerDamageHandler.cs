...
public class vp_PlayerDamageHandler : vp_DamageHandler
{
	...
	public override void Damage(float damage)
	{
		...
		m_Player.Armour.Set(currentArmour);
	...
	protected virtual float OnValue_Armour
	{
		get
		{
	...
	public virtual bool CanStart_Heal()
	{
		if(m_Player.GetItemCount.Send("HealthPack") <= 0)
		{
			m_Player.HUDText.Send("Out of Health Packs");
			return false;
		}

		if(m_Player.Health.Get() >= 1.0f)
		{
			m_Player.HUDText.Send("Health Full");
			return false;
		}

		m_Player.Health.Set(1.0f);
		m_Player.RemoveItem.Try(new object[]{"HealthPack"});
		return true;
	}
